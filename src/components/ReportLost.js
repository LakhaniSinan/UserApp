...
class ReportLost extends Component{
   ...
   submit(){
    ...
    if(name!==''&&contact!==''&&item!==''&&itemdetails!==''){
            let index=uid()
firebase.database().ref(`/lost/${index}`).set({name,contact,item,itemdetails,location,avatarSource})
console.log(index,'val of index')
this.setState({name:'',contact:'',item:'',itemdetails:'',location:'',avatarSource:null})
ToastAndroid.showWithGravity('Succesfully Reported',ToastAndroid.LONG,
